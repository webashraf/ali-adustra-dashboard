...
import TModal from '../../Home/Teacher/TModal/TModal';
...
const PublicNotice = () => {
    ...
    const {data: publicNotice = [], refetch} = useQuery({
        queryKey: [],
        queryFn: async () => {
            const res = await fetch(`http://localhost:3000/public-notice`)
            return res.json();
        }
    })
    console.log("Public Notice", publicNotice);
    const onSubmit = (data) => {
        refetch();
        console.log("Notice Data", data?.noticeType);
        const teacherNotice = {
            title: data?.title,
            notice: data?.notice,
            name: mUser?.userName,
    ...
    // console.log("Notice User", mUser);

    return (
        <div>
            <dialog id="my_modal_5" className="modal modal-bottom sm:modal-middle">
                <TModal currentNotice={singleNotice} nRefetch={refetch}></TModal>
            </dialog>
            <div className="pb-20">
                        ...
                        <div className="overflow-x-auto">
                            <table className="table">
                                ...
                                <tbody>
                                    ...
                                    {
                                        publicNotice.map((notice, index) => <tr key={notice._id} className="hover">
                                            <th>{index + 1}</th>
                                            <td>
                                                <div>{notice?.name}</div>
                                                <div>{notice?.email}</div>
                                                <div>{moment(notice?.postTime).format("MMMM Do YYYY")}</div>
                                            </td>
                                            <td>{notice?.title}</td>
                            ...
                            <form className="w-full max-w-lg mt-8" onSubmit={handleSubmit(onSubmit)}>
                                ...
                                <div className="-mx-3 mb-6">
                                    ...
                                    <div className="w-full px-3 mb-6 md:mb-0">
                                        ...
                                        <select name="noticeType" {...register("noticeType")}>
