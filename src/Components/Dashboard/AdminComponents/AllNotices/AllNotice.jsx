...
const AllNotice = () => {
    ...
    const { refetch: noticeRefetch, data: notices = [] } = useQuery({
        queryKey: [],
        queryFn: async () => {
            const res = await fetch(`http://localhost:3000/all-notice`)
            return res.json()
        }
    })
                        ...
                        <div className="overflow-x-auto">
                            <table className="table">
                                ...
                                <tbody>
                                    ...
                                    {
                                        notices.map((notice, index) => <tr key={notice._id} className="hover">
                                            <th>{index + 1}</th>
                                            <td>
                                                <div>{notice?.name}</div>
